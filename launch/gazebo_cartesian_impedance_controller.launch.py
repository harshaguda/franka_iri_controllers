...
def _robot_state_publisher_from_xacro(context: LaunchContext, arm_id, load_gripper, franka_hand):
...
def generate_launch_description():
    ...
    robot_state_publisher = OpaqueFunction(
        function=_robot_state_publisher_from_xacro,
    ...
    load_gazebo_cartesian_impedance_controller = ExecuteProcess(
        cmd=['ros2', 'control', 'load_controller', '--set-state', 'active', 'gazebo_cartesian_impedance_controller'],
        output='screen',
    )
        ...
        [
            ...
            RegisterEventHandler(
                event_handler=OnProcessExit(
                    ...
                    on_exit=[TimerAction(period=2.0, actions=[load_gazebo_cartesian_impedance_controller])],
                )
            ),
            Node(
